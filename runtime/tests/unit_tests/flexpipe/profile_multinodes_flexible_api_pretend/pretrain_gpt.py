...
def loss_func(loss_mask: torch.Tensor, output_tensor: torch.Tensor):
    ...
    if mpu.get_context_parallel_world_size() > 1:
        ...
        # print(f'{torch.distributed.get_rank()} - loss_func before all_reduce {torch.distributed.get_process_group_ranks(mpu.get_context_parallel_group())} {loss}')
        # torch.distributed.all_reduce(loss, group=mpu.get_context_parallel_group())
        # print(f'{torch.distributed.get_rank()} - loss_func after all_reduce')
        loss = loss[0] / loss[1]
    else:
    ...
    return loss * mpu.get_context_parallel_world_size(), {'lm loss': averaged_loss[0]}
...
def forward_step(data_iterator, model: FlexGPTModel, extra_tensors_):
    ...
    print(f'{torch.distributed.get_rank()} - forward_step')
    ...
    print(f'{torch.distributed.get_rank()} - forward_step before model')

    output_tensor, output_extra_tensors = model(input_tensors, input_extra_tensors)
    print(f'{torch.distributed.get_rank()} - forward_step end')
